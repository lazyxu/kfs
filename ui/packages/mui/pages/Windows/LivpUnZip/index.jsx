...
import { startLivePhotoAnalysisTask } from "@kfs/mui/api/exif";
...
export default function ({ id }) {
    ...
    useEffect(() => {
        setTaskInfo();
        fetchEventSource(`${getSysConfig().webServer}/api/v1/event/livePhotoAnalysisTask`, {
            ...
            async onopen(response) {
                ...
                noteError("event/livePhotoAnalysisTask.onopen: " + response.status);
            },
            onmessage(msg) {
                ...
                if (msg.event === 'FatalError') {
                    console.error(msg);
                    noteError("event/livePhotoAnalysisTask.onmessage: " + msg);
                    return;
                }
            ...
            onclose() {
                ...
                noteError("event/livePhotoAnalysisTask.onclose");
            },
            onerror(err) {
                console.error(err);
            ...
            <WorkingArea>
                <Box sx={{ padding: "0.5em 1em" }}>
                    {(taskInfo?.status === undefined ||
                        ...
                        <IconButton onClick={e => startLivePhotoAnalysisTask(true, true).catch(e => noteError(e.message))}>
                            <Replay />
                        </IconButton>
                    }
                    {(taskInfo?.status === undefined ||
                        ...
                        taskInfo?.status === StatusCanceled ||
                        taskInfo?.status === StatusError) &&
                        <IconButton onClick={e => startLivePhotoAnalysisTask(true, false).catch(e => noteError(e.message))}>
                            <PlayArrow />
                        </IconButton>
                    }
                    ...
                    {(taskInfo?.status === StatusRunningCollect ||
                        ...
                        <IconButton onClick={e => startLivePhotoAnalysisTask(false).catch(e => noteError(e.message))}>
                            <Stop />
                        </IconButton>
                    }
                    任务：livp文件解压缩，得到heic和mov文件
                </Box>
                <Box sx={{ padding: "0.5em 1em" }}>
