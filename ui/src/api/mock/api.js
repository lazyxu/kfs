...
export function open(sysConfig, branchName, path, onFile, onTotal, onDirItem) {
    console.log('mock.open', branchName, path, path.join('/'))
    ...
    for (let i = 0; i < dirItems.length; i++) {
        if (dirItems[i].Name === path[0]) {
            if (modeIsDir(dirItems[i].Mode)) {
                path.shift()
                return listR(dirItems[i].DirItems, path, onFile, onTotal, onDirItem)
            } else {
                let content = dirItems[i].Content;
...
export function list(sysConfig, branchName, path, onTotal, onDirItem) {
    console.log('mock.list', branchName, path, path.join('/'))
